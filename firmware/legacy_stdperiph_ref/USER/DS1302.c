...
/**********以下对时分秒的初始化*************/
        DS1302_Write(ds1302_sec_addr,DEC_BCD(s_sec));  //秒
        DS1302_Write(ds1302_min_addr,DEC_BCD(s_min));   //分
        DS1302_Write(ds1302_hour_addr,DEC_BCD(s_hour));   //时
      /**********以下对年月日的初始化*************/
     DS1302_Write(ds1302_day_addr,DEC_BCD(s_day));
   DS1302_Write(ds1302_month_addr,DEC_BCD(s_mon));
      DS1302_Write(ds1302_year_addr,DEC_BCD(s_year));
...
void DS1302_init(uchar s_year,uchar s_mon,uchar s_day,uchar s_hour,uchar s_min,uchar s_sec)
{
    ...
    {
        DS1302_SetTime(s_year,s_mon,s_day,s_hour,s_min,s_sec);//设置设置初始时钟
    }
    //否则跳过
